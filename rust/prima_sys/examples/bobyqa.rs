//! Example of using the BOBYQA optimization algorithm.
...
fn main() -> () {
    ...
    let mut variable_lower_bounds = vec![-1.0, -1.0];
    let mut variable_upper_bounds = vec![4.5, 4.5];
    ...
    unsafe {
        ...
        (*uninit_problem_ptr).xl = variable_lower_bounds.as_mut_ptr();
        (*uninit_problem_ptr).xu = variable_upper_bounds.as_mut_ptr();
        ...
        let _return_code = prima_minimize(
            prima_algorithm_t_PRIMA_BOBYQA,
            problem,
            options,
            uninit_result_ptr);
