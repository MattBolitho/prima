//! Example of using the LINCOA optimization algorithm.
...
fn main() -> () {
    ...
    let mut aineq = vec![1.0, 1.0];
    let mut bineq = vec![8.0];
    ...
    unsafe {
        ...
        (*uninit_problem_ptr).Aineq = aineq.as_mut_ptr();
        (*uninit_problem_ptr).bineq = bineq.as_mut_ptr();
        ...
        let _return_code = prima_minimize(
            prima_algorithm_t_PRIMA_LINCOA,
            problem,
            options,
            uninit_result_ptr);
